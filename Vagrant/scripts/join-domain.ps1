# Purpose: Joins a Windows host to the pma.corp.pvt domain which was created with "create-domain.ps1".
...
If ($hostname -eq "wef") {
  Add-Computer -DomainName "pma.corp.pvt" -credential $DomainCred -OUPath "ou=Servers,dc=pma,dc=corp,dc=pvt" -PassThru
  ...
  Add-Computer -DomainName "pma.corp.pvt" -credential $DomainCred -OUPath "ou=Workstations,dc=pma,dc=corp,dc=pvt"
} Else {
  Add-Computer -DomainName "pma.corp.pvt" -credential $DomainCred -PassThru
}
