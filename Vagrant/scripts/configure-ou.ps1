...
Add-Content "c:\windows\system32\drivers\etc\hosts" "        192.168.38.102    dc.pma.corp.pvt"
...
while ($servers_ou_created -ne 1) {
  ...
  try {
    Get-ADOrganizationalUnit -Identity 'OU=Servers,DC=pma,DC=corp,DC=pvt' | Out-Null
    Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Servers OU already exists. Moving On."
    $servers_ou_created = 1
  }
  catch [Microsoft.ActiveDirectory.Management.ADIdentityNotFoundException] {
    New-ADOrganizationalUnit -Name "Servers" -Server "dc.pma.corp.pvt"
...
Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Creating Workstations OU..."
  try {
    Get-ADOrganizationalUnit -Identity 'OU=Workstations,DC=pma,DC=corp,DC=pvt' | Out-Null
    Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Workstations OU already exists. Moving On."
    $workstations_ou_created = 1
  }
  catch [Microsoft.ActiveDirectory.Management.ADIdentityNotFoundException] {
    New-ADOrganizationalUnit -Name "Workstations" -Server "dc.pma.corp.pvt"
