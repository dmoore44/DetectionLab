...
Invoke-Command -computername dc -Credential (new-object pscredential("pma\vagrant", (ConvertTo-SecureString -AsPlainText -Force -String "vagrant"))) -ScriptBlock {

    Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) [$env:computername] Installing the ATA Lightweight gateway on DC..."

    Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) [$env:computername] Adding wef.pma.corp.pvt to hosts file..."
    Add-Content 'c:\\windows\\system32\\drivers\\etc\\hosts' '        192.168.38.103    wef.pma.corp.pvt'
