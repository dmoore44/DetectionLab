...
$gpLinks = $null
$OU = "OU=Servers,dc=pma,dc=corp,dc=pvt"

$gPLinks = Get-ADOrganizationalUnit -Server "dc.pma.corp.pvt" -Identity $OU -Properties name,distinguishedName, gPLink, gPOptions
...
$OU = "ou=Domain Controllers,dc=pma,dc=corp,dc=pvt"
$gpLinks = $null
$gPLinks = Get-ADOrganizationalUnit -Server "dc.pma.corp.pvt" -Identity $OU -Properties name,distinguishedName, gPLink, gPOptions
$GPO = Get-GPO -Name $GPOName
If ($gPLinks.LinkedGroupPolicyObjects -notcontains $gpo.path)
...
$OU = "ou=Workstations,dc=pma,dc=corp,dc=pvt"
$gpLinks = $null
$gPLinks = Get-ADOrganizationalUnit -Server "dc.pma.corp.pvt" -Identity $OU -Properties name,distinguishedName, gPLink, gPOptions
$GPO = Get-GPO -Name $GPOName
If ($gPLinks.LinkedGroupPolicyObjects -notcontains $gpo.path)
